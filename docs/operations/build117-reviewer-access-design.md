...
3. **Create `APPREVIEW117`** in prod via `POST /admin/coupon/create` with `anonymous_eligible: true`, **`duration_days: 60`** (the live ASC review notes promise "60 days of reviewer access" — verified 2026-08-05 via the ASC API), a **bounded `max_uses`** (e.g. 25) and a **near `expires_at`** (review window + buffer). Admin secret required. The redeem response message is already "Pro access activated!", matching the reviewer notes' expected confirmation.
4. **ASC metadata — VERIFY BEFORE CHANGING (2026-08-05 live check contradicts the reported defect):** querying the ASC API (key PSS5YP9VS4, app 6785755956) shows v1.1's `appStoreReviewDetail.contactEmail = Dov.Ginsburg@gmail.com` (resolving Gmail), `contactPhone = 9179712801` (real 917 number), and support/marketing/privacy URLs all on `tonoit.com`. **No `tonoit.app` address and no 555 phone were found** in the review-contact, version localizations, or app-info. The "non-resolving tonoit.app email + 555 phone" correction appears stale or about a different field — confirm the exact field in App Store Connect before editing; the review-facing fields checked here are already clean.
