...
# ---------------------------------------------------------------------------
# Challenge store — short-lived, single-use, in-memory (same "fine for a
# single-process SQLite deployment" reasoning as the rate-limit windows in
# server.py/slack.py; a durable store only matters once this runs as more
# than one worker, at which point Postgres + a shared cache is the bigger
# migration anyway — see ARCHITECTURE.md).
# ---------------------------------------------------------------------------

_registration_challenges: dict[str, tuple[bytes, float]] = {}  # device_id -> (challenge, expires_at)
_login_challenges: dict[str, float] = {}  # challenge (b64url) -> expires_at


def _put_registration_challenge(device_id: str, challenge: bytes) -> None:
    _registration_challenges[device_id] = (challenge, time.time() + _CHALLENGE_TTL_SECONDS)


def _pop_registration_challenge(device_id: str) -> bytes:
    entry = _registration_challenges.pop(device_id, None)
    if not entry or entry[1] < time.time():
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "registration challenge expired or missing — call options again")
    return entry[0]


def _put_login_challenge(challenge_b64: str) -> None:
    _login_challenges[challenge_b64] = time.time() + _CHALLENGE_TTL_SECONDS
    _prune_login_challenges()


def _pop_login_challenge(challenge_b64: str) -> None:
    expires_at = _login_challenges.pop(challenge_b64, None)
    if not expires_at or expires_at < time.time():
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "login challenge expired or missing — call options again")


def _prune_login_challenges() -> None:
    now = time.time()
    expired = [c for c, exp in _login_challenges.items() if exp < now]
    for c in expired:
        _login_challenges.pop(c, None)
...
async def passkey_register_options(user: CurrentUser, store: StoreDep):
    ...
    existing = store.list_webauthn_credentials(account.id)
    ...
    _put_registration_challenge(user.device_id, options.challenge)
...
async def passkey_register_verify(
    ...
    challenge = _pop_registration_challenge(user.device_id)
    try:
...
async def passkey_login_options():
    ...
    _put_login_challenge(bytes_to_base64url(options.challenge))
    return Response(content=options_to_json(options), media_type="application/json")
...
async def passkey_login_verify(
    ...
    _pop_login_challenge(challenge_b64)
    ...
    stored_credential = store.get_webauthn_credential(credential_id_b64)
    ...
    store.update_webauthn_sign_count(credential_id_b64, verification.new_sign_count)
    ...
    store.link_device_to_account(user.device_id, stored_credential.account_id)
    account = store.get_account(stored_credential.account_id)
...
async def passkey_list(user: CurrentUser, store: StoreDep) -> list[PasskeyListItem]:
    ...
    return [
        ...
        for c in store.list_webauthn_credentials(user.account_id)
    ]


@router.delete("/{credential_id}", status_code=status.HTTP_204_NO_CONTENT)
async def passkey_delete(credential_id: str, user: CurrentUser, store: StoreDep) -> Response:
    if not user.account_id or not store.delete_webauthn_credential(credential_id, user.account_id):
