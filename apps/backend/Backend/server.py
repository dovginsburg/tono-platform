...
def _get_client_ip(request: Request) -> str:
    """Resolve the caller's IP for `_check_ip_rate`.

    Deliberately trusts the LAST entry in `X-Forwarded-For`, not the
    first: a client connecting directly can set an arbitrary
    `X-Forwarded-For` header themselves, and a well-behaved reverse proxy
    (Railway/Fly, per this repo's Dockerfile/railway.toml/fly.toml —
    both single-hop) *appends* its own observed peer address rather than
    replacing the header, so the first entry is always attacker-
    controlled while the last is the one the trusted proxy actually
    added. Trusting the first entry (an earlier version of this function
    did) makes the per-IP rate limit trivially bypassable — send a random
    fake `X-Forwarded-For` value on every request and each one lands in
    its own fresh bucket, which defeats the exact "block scripted abuse
    before it touches an LLM" purpose this exists for.

    This assumes exactly one reverse proxy hop between the client and
    this process. Adding a second hop in front (e.g. a CDN) would need
    this to skip an additional trailing entry, or better, an explicit
    trusted-hop-count config — not needed for this deployment today.
    """
...
async def _check_ip_rate(ip: str) -> bool:
    """Sliding-window rate limiter. Returns False if the IP is over limit."""
    return await check_sliding_window(f"ip:{ip}", _IP_RATE_LIMIT, window_seconds=60)
...
def _analysis_cache_key(
    ...
    voice: str | None,
    locale: str,
    custom_axes: list[CustomAxis] | None = None,
) -> str:
    custom_part = ",".join(sorted(f"{c.name}:{c.instruction}" for c in (custom_axes or [])))
    raw = f"{text}|{','.join(sorted(axes))}|{custom_part}|{voice or ''}|{locale}"
...
async def _lifespan(_: "FastAPI"):
    await get_store().init_schema()  # creates tables + seeds feature flags
    logger.info(
...
async def register(body: RegisterRequest, store: StoreDep) -> RegisterResponse:
    user = await store.register_device(body.device_id)
...
async def _resolve_provider_signin(
    ...
    try:
        account = await store.upsert_account_by_provider(
...
async def auth_apple(
    ...
    account = await _resolve_provider_signin(store, user, "apple", claims.sub, claims.email, body.link)
...
async def auth_google(
    ...
    account = await _resolve_provider_signin(store, user, "google", claims.sub, claims.email, body.link)
    return SignInResponse(account_id=account.id, plan=account.plan, is_pro=account.is_pro, email=account.email)
...
async def api_analyze(
    ...
    if not await _check_ip_rate(client_ip):
        raise HTTPException(
    ...
    # `body.axes is None` means "the caller didn't specify" (fall back to
    # the crowd-wisdom ranking); `[]` is a deliberate "turn every preset
    # off" — those aren't the same thing, and `body.axes or ...` would
    # wrongly treat an explicit empty list as "unspecified" since `[]` is
    # falsy in Python.
    axes = body.axes if body.axes is not None else await store.global_axis_ranking(days=30)

    # Cache lookup — hits don't consume the daily allowance.
    cache_key = (
        _analysis_cache_key(body.text, axes, body.preferred_voice, body.locale, body.custom_axes)
        if provider != "mock"
        else None
    )
    if cache_key:
        cached = await store.get_cached_response(cache_key)
        if cached:
            today = _today_utc()
    ...
    allowed, used, limit = await store.consume_rewrite(user.device_id)
    if not allowed:
        await store.log_usage(user.device_id, "/api/analyze", 429, drafts_chars=len(body.text))
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail={
    ...
    if cache_key:
        await store.set_cached_response(cache_key, result)
    await store.log_usage(
...
async def log_axis_event(
    ...
    """
    await store.log_axis_event(user.device_id, body.axis, body.risk_level)
...
async def log_analytics_event(
    ...
    if body.event == "improvement_outcome":
        ...
        flags = await store.get_features(user.device_id, user.is_pro)
        if flags.get("improve_tono", True) and body.risk_level and body.msg_len_bucket:
            await store.log_improvement_event(
                device_id=user.device_id,
...
@app.get("/v1/features")
async def get_features(user: CurrentUser, store: StoreDep) -> dict[str, bool]:
    """Return the resolved feature flags for the authenticated device."""
    return await store.get_features(user.device_id, user.is_pro)
...
async def set_feature_preference(
    key: str,
    ...
    await store.set_user_flag_override(user.device_id, key, body.enabled, set_by="user")
    return {"ok": True, "key": key, "enabled": body.enabled}


@app.get("/v1/digest")
async def get_digest(user: CurrentUser, store: StoreDep) -> dict[str, Any]:
    """Weekly tone digest — rewrites, days active, axis breakdown."""
    return await store.get_weekly_digest(user.device_id)
...
@app.get("/admin/stats")
async def admin_stats(request: Request, store: StoreDep) -> dict[str, Any]:
    """Aggregate product analytics: axis usage, registration count, active pro count."""
    _check_admin(request)
    return await store.admin_summary_stats()
...
async def admin_axis_stats(
    ...
    """
    _check_admin(request)
    overall = await store.axis_stats(days=days)
    by_risk = await store.axis_stats_by_risk(days=days)
    ranking = await store.global_axis_ranking(days=days)
    return {
        "days": days,
        "overall": overall,
...
async def admin_improvement_stats(
    request: Request,
    store: StoreDep,
    days: int = 30,
    ...
    """
    _check_admin(request)
    effectiveness = await store.get_axis_effectiveness(days=days, min_devices=min_devices)
    quality = await store.get_rewrite_quality(days=days, min_devices=min_devices)
    return {
        "days": days,
...
async def admin_age_out_events(
    request: Request,
    store: StoreDep,
    retain_days: int = 90,
) -> dict[str, Any]:
    ...
    """
    _check_admin(request)
    deleted = await store.age_out_improvement_events(retain_days=retain_days)
...
@app.get("/admin/flags")
async def admin_list_flags(request: Request, store: StoreDep) -> list[dict[str, Any]]:
    """List all feature flags with their current state."""
    _check_admin(request)
    return await store.get_all_flags()
...
async def admin_set_flag_override(
    key: str,
    ...
    _check_admin(request)
    await store.set_user_flag_override(body.device_id, key, body.enabled, set_by="admin")
...
async def admin_delete_flag_override(
    key: str,
    device_id: str,
    request: Request,
    store: StoreDep,
    ...
    _check_admin(request)
    await store.delete_user_flag_override(device_id, key)
...
async def redeem_coupon(
    ...
    try:
        exp = await store.redeem_coupon(user.device_id, body.code.strip().upper())
