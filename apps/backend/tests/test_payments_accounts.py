...
async def test_checkout_uses_account_customer_when_signed_in(client, store, monkeypatch):
    ...
    device_row = await store.get_by_device(device["device_id"])
    assert device_row.stripe_customer_id is None


async def test_checkout_bills_device_when_anonymous(client, store, monkeypatch):
...
async def test_portal_uses_account_customer_when_signed_in(client, store, monkeypatch):
    from Backend.server import app
    ...
    await store.attach_account_stripe_customer(account_id, "cus_portal_1")
...
async def test_webhook_checkout_completed_updates_account_and_all_its_devices(client, store, monkeypatch):
