...
async def test_second_device_inherits_pro_from_shared_account(client, store):
...
# ---------------------------------------------------------------------------
# Concurrency: two devices signing in with the SAME brand-new identity at
# the same instant — a real race, not just two sequential calls.
# ---------------------------------------------------------------------------


async def test_concurrent_first_signin_with_same_identity_resolves_to_one_account(store):
    """Two devices both discovering a brand-new Apple identity for the
    first time, at the same instant, must not create two separate
    accounts (an orphaned duplicate) or blow up with an unhandled
    IntegrityError on the accounts.apple_sub unique constraint — the
    second writer should just see the first writer's account and return
    that instead. Exercises the store directly (not via the HTTP
    endpoints) since that's the only way to force two calls to actually
    overlap rather than just run sequentially fast."""
    import asyncio

    results = await asyncio.gather(
        store.upsert_account_by_provider("apple", sub="race-identity-1", email="race@example.com"),
        store.upsert_account_by_provider("apple", sub="race-identity-1", email="race@example.com"),
    )
    assert results[0].id == results[1].id

    accounts_with_this_sub = 0
    async with store._engine.begin() as conn:
        from sqlalchemy import select

        from Backend.db import accounts as accounts_table

        rows = (
            await conn.execute(select(accounts_table).where(accounts_table.c.apple_sub == "race-identity-1"))
        ).mappings().all()
        accounts_with_this_sub = len(rows)
    assert accounts_with_this_sub == 1
