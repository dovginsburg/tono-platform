# Rollback notes — tonoit-release-control-v5
...
Because the entire change is confined to `packages/tonoit-release-control-v5/`,
rollback is a single-commit revert:

```bash
git revert <this-commit-sha>
# or, to delete the directory outright on a follow-up branch:
git rm -r packages/tonoit-release-control-v5
```
...
grep -rN "tonoit_release_control_v5" --include='*.py' . | grep -v packages/tonoit-release-control-v5
