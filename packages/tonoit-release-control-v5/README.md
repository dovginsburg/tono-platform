# tonoit-release-control-v5

An **inert, stdlib-only, additive** Python package: the release-control
*strict-TTL malformed-model successor (v5)*. It is imported by nothing in the platform and
...
| Strict finite TTL | TTL must be a finite, strictly-positive real; future issuance and age `>= ttl_seconds` default off |
...
```bash
cd packages/tonoit-release-control-v5
...
PYTHONPATH=. python3 -m tonoit_release_control_v5.safety_demo
```
